...
class TestConstructChatPrompt:
    """construct_chat_prompt 関数のテスト"""

    def test_construct_chat_prompt_with_history(self):
        """
        会話履歴が含まれたプロンプトが構築されること
        """
        from api.ai import construct_chat_prompt

        schema = {}
        system_prompt = "チャットアシスタント"
        history = [
            {"role": "user", "content": "こんにちは"},
            {"role": "assistant", "content": "こんにちは！"},
        ]

        result = construct_chat_prompt(
            "今日の天気は？", schema, system_prompt, session_history=history
        )

        # 履歴がプロンプトに反映されること
        assert isinstance(result, str)
        assert "こんにちは" in result
        assert "今日の天気は？" in result

    def test_construct_chat_prompt_no_history(self):
        """
        履歴なしでもプロンプトが構築されること
        """
        from api.ai import construct_chat_prompt
        ...
        result = construct_chat_prompt("質問です", schema, system_prompt)
